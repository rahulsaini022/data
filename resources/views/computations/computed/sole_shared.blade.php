...
<div class="row">
 <table width="100%" border="0">
  ...
  <tr class="hide-print-buttons">
    <td align="left" width="50%" style="float: left;padding-top: 15px;">
      <form method="POST" action="{{route('ajax_update_custody_arrangement_dr_children')}}" autocomplete="off">
          ...
          <input class="textcenter" type="hidden" name="obligee_name_custody" placeholder="Enter" value="<?php if(isset($postData['client_full_name'])){ echo $postData['client_full_name']; } else if(isset($case_data['client_full_name'])){ echo $case_data['client_full_name']; } ?>">
          <input class="textcenter" type="hidden" name="obligor_name_custody" placeholder="Enter" value="<?php if(isset($postData['opponent_full_name'])){ echo $postData['opponent_full_name']; } else if(isset($case_data['opponent_full_name'])){ echo $case_data['opponent_full_name']; } ?>">
...
<form method="post" action="{{ route('computed_computations.sole-shared') }}" onsubmit="return validateSoleForm();" id="sole_shared_form">
  @csrf
...
<table border="1" bordercolor="#333" class="Tableworksheet table_outer income-share cash-medical page-3" style="border-bottom:none">
        ...
        <tr>
          ...
          <td colspan="2"><div class="input_field_wrapper hide-inputbtns"><input data-number-stepfactor="100" step="1" class="textright currency" type="number" id="obligor_32" name="obligor_32" placeholder="Calculate"  value="<?php echo isset($postData['obligor_32'])?number_format((float)$postData['obligor_32'], 2, '.', ''):''; ?>" readonly></div></td>
        </tr>
...
<script type="text/javascript">
  ...
  function switchObligorObligee(){
    ...
    if(typeof obligor_3_c_top_override_input !== "undefined")
    {
     ...
     // to switch councel for between obligee/obligor
    var councel_for=$('select[name=counsel_dropdown]').val();
    if(councel_for=='obligee'){
      $('select[name=counsel_dropdown] option:selected').removeAttr('selected');
      $('select[name=counsel_dropdown] option[value=obligor]').prop('selected', true);
    }
    if(councel_for=='obligor'){
      $('select[name=counsel_dropdown] option:selected').removeAttr('selected');
      $('select[name=counsel_dropdown] option[value=obligee]').prop('selected', true);
    }
