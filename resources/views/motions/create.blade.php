...
<div class="container">
    <div class="row justify-content-center motions-main">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header"><strong>{{ __('Register New Motion') }}</strong>
                  <div class="pull-right">

                        <a class="btn btn-info" href="{{ route('cases.pleadings',['case_id' => $case_data->id]) }}"> Go to Pleadings</a>
                ...
                <div class="card-body table-sm table-responsive">
                    ...
                    @else 
                    <div id="motion-drafting-section" class="col-sm-12 motion-without-third-parties">
                    @endif
                      <form id="motion_form" method="POST" action="{{route('cases.motions.store')}}" autocomplete="off">
                          @csrf
                          <input type="hidden" name="case_id" value="{{$case_data->id}}">
                          <input type="hidden" name="motion_category" value="New Core Motion">
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            @endif
                                    @foreach ($bottom_party_data as $key => $party)
                                        <div class="check-box-options">
                                            <label class="label_movants_{{ $party->id }}"><input type="checkbox" class="right_movant all_movant_bottom all_movant_bottom_party" id="left_movant_{{ $party->id }}" name="movants[]" value="{{ $party->id }}" onchange="onMovantChange(this);" data-user-name="{{ $party->name }}" data-user-type="bottom" data-user-party="{{$bottom_party_type}}"> {{ $party->name }}</label>
                                        </div>
                                        <div class="check-box-options">
                            ...
                            @endif
                                    ...
                                    @foreach ($bottom_third_party_data as $key => $party)
                                        <div class="check-box-options">
                                            <label class="label_movants_{{ $party->id }}"><input type="checkbox" class="right_movant all_movant_third all_movant_third_party" id="left_movant_{{ $party->id }}" name="movants[]" value="{{ $party->id }}" onchange="onMovantChange(this);" data-user-name="{{ $party->name }}" data-user-type="third" data-user-party="Third-Party {{$bottom_party_type}}"> {{ $party->name }}</label>
                                        </div>
                                        <div class="check-box-options">
                                            <label class="label_movants_{{ $party->id }}"> N/A</label>
                                        </div>
...
<script type="text/javascript">
    function onMovantChange(movant){
        ...
        // var totalcheck=$('[name="movants[]"]:checked').length;
        // if(totalcheck > 0){
        //     if(movant.id=='left_movant_'+movant.value+'' && $('#left_movant_'+movant.value+'').is(":checked")){
        //         $('.movant_clients').prop('disabled', true);
        //     }
        //     if($('input:checkbox.left_movants:checked').length == 0 && $('input:checkbox.left_res:checked').length > 0){
        //         $('.movant_clients').prop('disabled', false);
        //     }
        // } else{
        //     $('.movant_clients').prop('disabled', false);
        // }
