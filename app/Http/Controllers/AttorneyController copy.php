...
class AttorneyController extends Controller
{
  ...
  public function getSeatLicensePaymentForm()
  {
    ...
    $advertise_id = ($user->advertiser->id) ? $user->advertiser->id : 0;
  ...
  {
    $ads_categories = AdvertiserService::where('parent_id', 0)->get();
    return view('attorney.adsCategory', compact('ads_categories'));
  }
  /**advertiser service category details*/
  public function AdDetails()
  {
   
    try {
      $attorney_county=county::where('id',auth()->user()->attorney->county_id)->first()->county_name;

      $listing = DB::table('advertiser_listings')->where('county_id', auth()->user()->attorney->county_id)->whereNotIn('status', [0])->get();
      ...
      $counties = DB::table('advertiser_listings')->join('counties', 'counties.id', '=', 'advertiser_listings.county_id')->select('counties.id', 'counties.county_name')->where('advertiser_listings.status',1)->groupBy('advertiser_listings.county_id')->get();
      ...
      return view('attorney.adsDetails', compact('listing','attorney_county','counties','categories'));
    } catch (Exception $e) {
      return redirect()->route('ads_category')->with('error', 'Category not found');
    }
  }
  public function Filtering(Request $request)
  {
    ...
    $counties = DB::table('advertiser_listings')->join('counties', 'counties.id', '=', 'advertiser_listings.county_id')->select('counties.id', 'counties.county_name')->groupBy('advertiser_listings.county_id')->orderByDesc('counties.county_name')->get();
    $categories = DB::table('advertiser_listings')->join('advertiser_services', 'advertiser_services.id', '=', 'advertiser_listings.advertise_category_id')->select('advertiser_services.id', 'advertiser_services.name')->where('advertiser_services.parent_id', 0)->groupBy('advertiser_listings.advertise_category_id')->get();
    return view('attorney.adsDetails', compact('attorney_county','listing', 'advertise_id', 'counties', 'categories', 'county', 'category'));
  }
}
