...
class ComputationController extends Controller
{
      ...
      /* Save/Download Sole Shared Computation Sheet */
      public function soleSharedSheet(Request $request)
      {
            ...
            if((isset($request->save_form) && $request->save_form=='Save') || (isset($request->download_form) && $request->download_form=='Download'))
            {
                  ...
                  $array=array(
                          ...
                          "form_custody"=>$request->sheet_custody,
                              ...
                              // dd($array);
                  $users_attorney_submissions_id=NULL;
                  if(isset($request->case_id)){
                        ...
                        if(isset($request->users_attorney_submissions_id) && $request->users_attorney_submissions_id !=''){
                              DB::table('users_attorney_submissions')->where('id',$request->users_attorney_submissions_id)->update($array);
                              $users_attorney_submissions_id=$request->users_attorney_submissions_id;
                        } else {
                              // die('no');
                              $users_attorney_submissions_id=DB::table('users_attorney_submissions')->insertGetId($array);
                        }
                  } else {
                  ...
                  $array2=array(
                              ...
                              "obligor_32"=>($postData['obligor_32'] > 0 ) ? $postData['obligor_32'] : '0.00',
                              ...
                              //"OhSSCSTrig"=>'1',
                              "users_attorney_submissions_id"=>$users_attorney_submissions_id,
                              "updated_at"=>now(),
                          );
                  ...
                  if(isset($request->case_id)){
                    ...
                    $sheet_data=DB::table('sole_shared_submissions')->where('users_attorney_submissions_id', $users_attorney_submissions_id)->first();
                        if(isset($sheet_data)){
                              DB::table('sole_shared_submissions')->where('users_attorney_submissions_id',$users_attorney_submissions_id)->update($array2);
                        } else {
                              // die('no');
                  ...
                  if(isset($request->case_id)){
                        $prefill_data = DB::table('users_attorney_submissions')
                                    ->where('id',$users_attorney_submissions_id)
                                    ->get()->pluck('form_text');
                  } else {
                        $prefill_data = DB::table('users_attorney_submissions')
                                    ->where('id',$users_attorney_submissions_id)
                  ...
                  $OH_Minimum_Wage = $ohMinimumWageData[0]->tmpResult;
                  $postData['OH_Minimum_Wage']=$OH_Minimum_Wage;
                  $postData['users_attorney_submissions_id']=$users_attorney_submissions_id;
                  ...
                  if(isset($request->download_form) && $request->download_form=='Download')
                  {
                     $triggers_all_packages = DB::table('triggers_all_packages')->insert(
                       ['attorney_self_id' => Auth::user()->id, 'trig_package' => 'Ohio Quick Sole-Shared Child Support Worksheet','package_select'=> 1,'csss_id'=>$users_attorney_submissions_id]
                      );
                      $user_id = Auth::user()->id;
                      ...
                      $votes=DB::table('FDD_triggers_all_documents_Votes')->get();
                        //sleep(2);
                        //$votes=DB::table('FDD_View_Quick_CSSS_PDF_Votes')->first();
                        // exec('sh /home/ubuntu/.BFLODocs/MacrosScripts/QuickOhCSSS_pdf.sh', $output, $return); // old script
                        //exec('touch /var/www/html/public/LatchProcessing/LatchVotes/FDD_View_Quick_CSSS_PDF/junk.txt', $output, $return); // new script
                        ...
                        if (!$return)
                        {
                              ...
                              if(isset($request->obligee_name) && $request->obligee_name !=''){
                                  ...
                                  $client_fname =$client_fullname[0];
                              ...
                              // dd($data);
                              $path=( public_path() . '/uiodirs/'.Auth::user()->id.'/download/'.$client_fname.'_'.$data->state.'_CS_'.$data->form_custody.'_'.$datetime.'.pdf');
                              $headers = array(
                                    'Content-Type'=> 'application/pdf'
                                  );
                              $filename=$client_fname.'_'.$data->state.'_CS_'.$data->form_custody.'_'.$datetime.'.pdf';
                    ...
                    if($postData['obligor_32'] < 0){
                    $postData['obligor_32'] = '0.00';
                  }

                  return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ])->with('success', 'Computation sheet saved successfully.');
            ...
            // if(isset($request->submit) && $request->submit=='Calculate' || )
            {
                  ...
                  if(isset($request->submit_email) && $request->submit_email=='Print'){
                        return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ])->with('print_sheet', '1');
                  }

                  return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ])->with('bottom_scroll', '');

            }


            // if($request->chk_prefill=='1'){
            if(isset($request->prefill_selected_computation)){
                        $prefill_data = DB::table('users_attorney_submissions')
                                    ->where('id', $request->prefill_selected_computation)
                                    ->get()->pluck('form_text');
                  if(isset($prefill_data[0])){
                        ...
                        $OH_Minimum_Wage = $ohMinimumWageData[0]->tmpResult;
                        $postData['OH_Minimum_Wage']=$OH_Minimum_Wage;
                        $postData['users_attorney_submissions_id']=$request->prefill_selected_computation;
                        return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ]);
                  } else{
                        $ohMinimumWageData=DB::select(DB::raw("select getOHMinimumWage2018(0) AS tmpResult"));
                        $OH_Minimum_Wage = $ohMinimumWageData[0]->tmpResult;
                        return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'attorney_data'=>$attorney_data, 'OH_Minimum_Wage'=>$OH_Minimum_Wage]);
                  }                                    
                  
            }
            $ohMinimumWageData=DB::select(DB::raw("select getOHMinimumWage2018(0) AS tmpResult"));
            $OH_Minimum_Wage = $ohMinimumWageData[0]->tmpResult;
            return view('computations.sole_shared',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'attorney_data'=>$attorney_data, 'case_data'=>$case_data, 'OH_Minimum_Wage'=>$OH_Minimum_Wage]);
      ...
      /* Save/Download Split Computation Sheet */
      public function splitSheet(Request $request)
      {
            ...
            if(isset($request->submit_email) && $request->submit_email=='Print')
            {
                  ...
                  */
                  ...
                  return view('computations.split_print',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'postData'=>$postData, 'attorney_data'=>$attorney_data ]);
            }
            ...
            if((isset($request->save_form) && $request->save_form=='Save') || (isset($request->download_form) && $request->download_form=='Download'))
            {
                  ...
                  */
                  ...
                  $array2=array(
                              ...
                              "parentb_32"=>($postData['obligor_32'] > 0 ) ? $postData['obligor_32'] : '0.00',  
                  ...
                  if(isset($request->case_id)){
                    ...
                    $sheet_data=DB::table('split_submissions')->where('users_attorney_submissions_id', $users_attorney_submissions_id)->first();
                        if(isset($sheet_data)){
                              DB::table('split_submissions')->where('users_attorney_submissions_id',$users_attorney_submissions_id)->update($array2);
                        } else {
                              // die('no');
                  ...
                  if($users_attorney_submissions_id){
                    $postData['users_attorney_submissions_id'] = $users_attorney_submissions_id;
                  }else{
                    $postData['users_attorney_submissions_id'] = DB::table('split_submissions')->latest('split_submissions.id')
                                  ->first();
                  }
                  ...
                  {
                      $triggers_all_packages = DB::table('triggers_all_packages')->insert(
                       ['attorney_self_id' => Auth::user()->id, 'trig_package' => 'Ohio Quick Split Child Support Worksheet','package_select'=> 1,'cssp_id'=>$users_attorney_submissions_id]
                      );
                      $user_id = Auth::user()->id;
                      /* call the procedure to update the package_select 0 */ 
                       DB::select("CALL packages2docs(?,?)",[$user_id,0]);
                      ...
                      $votes=DB::table('FDD_triggers_all_documents_Votes')->get();
                        //sleep(2);
                       // $votes=DB::table('FDD_View_Quick_CSSP_PDF_Votes')->first();
                        // exec('sh /home/ubuntu/.BFLODocs/MacrosScripts/QuickOhCSS_pdf.sh', $output, $return); // old script
                        //exec('touch /var/www/html/public/LatchProcessing/LatchVotes/FDD_View_Quick_CSSP_PDF/junk.txt', $output, $return); // new script
                        // Return will return non-zero upon an error
                        foreach($votes as $vote){
                          exec('touch '.$vote->vote_dir.'', $output, $return);
                        }
                  ...
                  return view('computations.split',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'postData'=>$postData, 'case_data'=>$case_data, 'attorney_data'=>$attorney_data ])->with('success', 'Computation sheet saved successfully.');            
            }      
            ...
            if(isset($request->submit) && $request->submit=='Calculate')
            {
                  ...
                  */
                  ...
                  return view('computations.split',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ])->with('bottom_scroll', '');
            }
            // if($request->chk_prefill=='1'){
             if(isset($request->prefill_selected_computation)){
                        $prefill_data = DB::table('users_attorney_submissions')
                  ...
                  if(isset($prefill_data[0])){
                        ...
                        $postData['OH_Minimum_Wage']=$OH_Minimum_Wage;
                        $postData['users_attorney_submissions_id']=$request->prefill_selected_computation;
                        return view('computations.split',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'postData'=>$postData, 'attorney_data'=>$attorney_data ]);
                  } else {
                        $ohMinimumWageData=DB::select(DB::raw("select getOHMinimumWage2018(0) AS tmpResult"));
                        $OH_Minimum_Wage = $ohMinimumWageData[0]->tmpResult;
                        return view('computations.split',['sheet_custody' =>$request->sheet_custody, 'sheet_state' =>$request->sheet_state, 'chk_prefill'=>$request->chk_prefill, 'case_data'=>$case_data, 'attorney_data'=>$attorney_data, 'OH_Minimum_Wage'=>$OH_Minimum_Wage]);
                  }
      ...
      public function deactivate($id){
          $array1=['active' => '0'];
          $users_attorney_submissions_data =DB::table('users_attorney_submissions')
                                  ->where('id', $id)
                                  ->get()
                                  ->first();
          if($users_attorney_submissions_data->user_id == Auth::user()->id){
          } else {
              return redirect()->route('home');
          }
          DB::table('users_attorney_submissions')->where('id', $id)->update($array1);

          return redirect()->back()->with('success','Computation Sheet deactivated successfully!');
      }
      ...
      public function activate($id){
          ...
          return redirect()->back()->with('success','Computation Sheet activated successfully!');
      }
