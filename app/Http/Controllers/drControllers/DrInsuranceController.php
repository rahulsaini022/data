...
class DrInsuranceController extends Controller
{
    /**
    ...
    {
        $data=DrInsurance::orderBy('id','DESC')->get();
    ...
    {   
        ...
        $DrInsurance=DrInsurance::where('case_id',$case_id)->get()->pluck('case_id');
        if(isset($DrInsurance['0'])){
            return redirect()->route('home');
        }
        ...
        return view('dr_tables.dr_Insurance.create',['case_id'=> $case_id, 'client_name' => $client_name, 'opponent_name' => $opponent_name]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $DrInsurance=DrInsurance::create($result);
        
        return redirect()->route('cases.family_law_interview_tabs',$result['case_id'])->with('success', 'Insurance Information Submitted Successfully.');
    ...
    public function edit($case_id)
    {
        ...
        $drinsurance=DrInsurance::where('case_id',$case_id)->get()->first();
        if($drinsurance){
            $caseuser=DB::table('caseusers')
                ->join('users', 'caseusers.user_id', '=', 'users.id')
                ->where([['caseusers.case_id', $case_id],['caseusers.party_group', 'top']])
            ...
            return view('dr_tables.dr_Insurance.edit',['case_id'=> $case_id, 'drinsurance' => $drinsurance, 'client_name' => $client_name, 'opponent_name' => $opponent_name]);
        } else {
            return redirect()->route('home');
        }
         // echo "<pre>";print_r($drinsurance);//die;
    ...
    public function update(Request $request, $id)
    {
        ...
        // echo "<pre>";print_r($result);die;
        $DrInsurance  = DrInsurance::findOrFail($id);
        if($DrInsurance){
            $DrInsurance->fill($result)->save();
            return redirect()->route('drinsurance.edit',$result['case_id'])->with('success', 'Insurance Information Updated Successfully.');
        } else {
            return redirect()->route('drinsurance.edit',$result['case_id'])->with('error', 'Something went wrong. Please try Again.');
        }
    }
