                ...
                <div class="card-body">
                    ...
                    @endif
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-1">
                                <input id="trial_attorney" type="checkbox" class="form-control @error('trial_attorney') is-invalid @enderror" name="trial_attorney" value="Yes" style="width: 15px;" <?php //if ($t=='1'){ echo "onclick='return false' checked"; } ?>>
                            </div>
                            @endif
                            @if(isset($total_customer_attorney) && $total_customer_attorney=='0')
                            <label for="customer_attorney" class="col-md-2 col-form-label text-md-left">{{ $customer_attorney_info->document_sign_name }}</label>

                            <div class="col-md-1">
                                <input id="customer_attorney" type="checkbox" class="form-control" name="customer_attorney" value="Yes" style="width: 15px;" <?php //if ($t=='1'){ echo "onclick='return false' data-onload='isCustomerAttorney(this)' checked"; } else { echo "onchange='isCustomerAttorney(this)'"; } ?> >
                            </div>
                            @endif
                        </div>
                        <div class="form-group row">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#customer_attorney').on('checked', function(){
            $("#customer_attorney").prop('checked', true);
            $('#noattorney').hide();
            var reg_num={{ Auth::user()->id  }};
            $('#attorney_reg_form').trigger("reset");
            $('#attorney_reg_1_num').val(reg_num);
            var state_id=$('#attorney_reg_1_state_id').val();
            if(reg_num){
                var token= $('input[name=_token]').val();
                $.ajax({
                    url:"{{route('ajax_get_attorney_by_checkbox')}}",
                    method:"POST",
                    dataType: 'json',
                    data:{
                        reg_num: reg_num, 
