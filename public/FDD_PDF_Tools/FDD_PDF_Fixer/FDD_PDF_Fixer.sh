...
shopt -u nullglob # Unsets nullglob
	...
	uniquefilename="$1"_"$current_time"
		#replace filename
		cp "$SOURCE"/"$bnwext" "$WORKDIR"/"$uniquefilename"_FDD_TEMP_comp.pdf
...
if [ $DIST != "None" ]
	then
			...
			mv "$WORKDIR"/"$uniquefilename"_"$bnwext" "$DIST"/"$uniquefilename"_"$bnFILENAME"_FDDFX.pdf
	else
			...
			mv "$WORKDIR"/"$bnwext" "$PROCESSED"/"$bnwext"
fi	
		#cleanup workdir
